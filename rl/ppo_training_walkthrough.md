...
The walkthrough above describes **synchronous PPO**: each phase blocks until the previous one completes,
so the GPUs alternate between being idle during rollout (when training workers wait) and idle during
training (when generation workers wait). Async PPO breaks this barrier by running generation and
training concurrently.

### 9.1 Motivation: GPU Utilization in Sync vs Async

In sync mode, a single training step looks like:

```
[────────────── Rollout (slow, ~80% of wall clock) ──────────────][Reward][LogP][Ref][Value][GAE][Train][WeightSync]
 Training workers: IDLE ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
 Rollout workers:                                                                       IDLE ▲▲▲▲▲▲▲▲▲▲▲
```

Rollout dominates because autoregressive generation is serial (one token per step), while a training
forward+backward is massively parallelized. Typical split: generation takes ~80% of wall-clock time per
iteration.

Async PPO overlaps the two:

```
Gen workers:   [─── gen batch 1 ───][─── gen batch 2 ───][─── gen batch 3 ───] ...
Train workers:          [─ train batch 1 ─][─ train batch 2 ─][─ train batch 3 ─] ...
                        ↑                  ↑                  ↑
                  weight sync         weight sync         weight sync   (every K train steps)
```

Generation workers push completed sequences to a shared buffer; training workers pull from it as soon
as enough data is available. Neither side idles waiting for the other.
...
**Generation workers** run an `AsyncLLMEngine` (vLLM or SGLang) with continuous batching. They accept
new prompts and stream out completed sequences without waiting for a global batch barrier. They use a
potentially stale copy of the actor weights.

**Training workers** run the same FSDP actor+critic setup as in the sync walkthrough (Phases 4–10).
They pull a mini-batch from the buffer once `buffer_size ≥ M` and broadcast updated weights back to
generation workers every K training steps.

**Data buffer** is a bounded queue. If the buffer fills, generation workers backpressure until training
catches up. If the buffer empties, training workers block until new sequences arrive.

### 9.3 The Staleness Problem

Because training workers update the policy independently of generation, sequences in the buffer were
generated by a policy that is **up to K training steps old**. In sync PPO, the behavior policy (π_old)
and the current training policy (π_new) differ only within a single mini-batch. In async PPO they can
be K updates apart.

This violates PPO's assumption that the importance ratio `π_new / π_old` stays close to 1.

**Importance sampling correction**: each sequence in the buffer carries `log_prob_behavior` — the
log prob recorded at generation time under the weights that produced it. The PPO surrogate uses this
instead of a separately-stored `old_log_probs`:

```
# log_prob_behavior stored in buffer at generation time
...
actor_loss = -mean(min(surr1, surr2) * response_mask)
```

The formula is identical to sync PPO; the difference is that `rho` can be meaningfully far from 1
when K is large.

**Staleness bound**: verl exposes a `max_policy_lag` hyperparameter. If generation workers' weights
are more than `max_policy_lag` training steps behind, generation pauses until the next weight
broadcast catches up. This bounds how far off-policy the buffer data can be.

```
max_policy_lag = K   # typical: 1–4 training steps
```

With K=1, data is at most 1 step stale — nearly on-policy. With K=4, the IS ratio distribution
widens and the clipped surrogate objective is under more stress.

### 9.4 Weight Synchronization Cadence

In sync PPO, weight sync happens once per step (Phase 10): `~3.5 GB` AllGather per training iteration.

In async PPO the sync is decoupled from the training step:

```
Every K training steps:
  Step 1 — Training workers: FSDP AllGather → full TP-sharded weights
    NCCL — AllGather across DP group:
      input per GPU:   ~875 MB FSDP shard (bfloat16)
      output per GPU:  ~3.5 GB full TP=2 copy (bfloat16)
      volume: same as Phase 10

  Step 2 — Ray broadcasts weights to generation workers via object store:
      payload: ~3.5 GB (once per K steps)

  Step 3 — Generation workers load new weights into AsyncLLMEngine
      (brief pause to swap parameters; in-flight sequences may be drained first)
```

Amortized per training step: `3.5 GB / K`. With K=4 that is ~875 MB/step vs 3.5 GB/step in sync
mode — a 4× reduction in weight-sync bandwidth.

### 9.5 Phase Correspondence

The per-sequence computation is the same as the sync walkthrough. The structural difference is
**when** and **on what** each phase runs.

| Sync phase | Async equivalent |
|-----------|-----------------|
| Phase 1 — Data loading | Controller streams prompts to gen workers continuously |
| Phase 2 — Rollout | Gen workers, running concurrently with training |
| Phase 3 — Reward | Applied before sequences enter the buffer |
| Phase 4 — Old log-probs | Recorded at generation time; stored in buffer as `log_prob_behavior` |
| Phase 5 — Ref log-probs | Ref workers run concurrently with generation (or collocated) |
| Phase 6 — Value estimation | Critic forward on training workers when batch is pulled |
| Phase 7 — GAE | Controller CPU, same as sync |
| Phase 8 — Actor update | Training workers, same FSDP/TP mechanics as sync |
| Phase 9 — Critic update | Training workers, same as sync |
| Phase 10 — Weight sync | Every K steps instead of every step |

### 9.6 Throughput vs. Convergence Trade-off

| Dimension | Sync PPO | Async K=1 | Async K=4 |
|-----------|----------|-----------|-----------|
| GPU utilization | ~20–40% | ~80–90% | ~85–95% |
| Policy staleness | 0 | 1 step | up to 4 steps |
| Weight sync bandwidth (per step) | 3.5 GB | 3.5 GB | ~875 MB |
| IS ratio spread | narrow | slightly wider | wider |
| Sample efficiency | highest | slightly lower | lower |
| Throughput (seqs/s) | 1× | ~3–5× | ~4–6× |

For tasks where rollout dominates (long responses, large models), async PPO with K=2–4 is the
standard choice. For tasks where training dominates (many PPO epochs, large critic, short responses),
sync PPO may match throughput with better sample efficiency.

### 9.7 Key Code Pointers (Async)

| Concept | File | Notes |
|---------|------|-------|
| Async PPO trainer | `verl/trainer/ppo/ray_trainer.py` | `AsyncPPOTrainer` class |
| AsyncLLMEngine rollout | `verl/workers/rollout/vllm_rollout/` | continuous batching via vLLM |
| Weight broadcast | `verl/trainer/ppo/ray_trainer.py` | `broadcast_weights_to_rollout()` |
| Data buffer / async queue | `verl/trainer/ppo/ray_trainer.py` | bounded sequence queue |
| Staleness config | `verl/trainer/config/` | `async_config.max_policy_lag` |
