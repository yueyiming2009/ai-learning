...
| 03 | **Transformer & Pretraining** — `docs/transformer/` / `#transformer` / `domain-transformer` / violet | Architecture · Training | assembling the pieces into a working model and training it: attention, the block, the layer variants (linear attn, DeltaNet, MoE routing), the next-token objective, and how to **size** the run (scaling laws) |
| 04 | **Systems & Scaling** — `docs/systems/` / `#systems` / `domain-systems` / orange | Performance & Kernels · Communication & Parallelism · Distributed Training | the substrate that trains it across many devices: kernels, MFU, collectives, parallelism, distributed plumbing |
